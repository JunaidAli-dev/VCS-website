...
export default async function Page() {
  ...
  // Query users/registrations/scores directly on the server
  const [{ data: usersData }, { data: regsData }, { data: scoresData }] =
    await Promise.all([
      supabase
        .from("users")
        .select(
          "email, name, image, role, full_name, institution, current_year, course, branch, phone_number"
        ),
  ...
  const usersList = (usersData || []).map((u: any) => ({
    ...
    registered_course_id:
      (regsData || []).find((r: any) => r.user_email === u.email)?.course_id ??
      null,
    scores: (scoresData || [])
      .filter((s: any) => s.user_email === u.email)
      .reduce(
  ...
  // add 99 as special Module 4 Quiz week
  const weeks = [1, 2, 3, 4, 5, 99];

  const getWeekLabel = (w: number) => {
    if (w === 99) return "Module 4 Quiz";
    return `Week ${w}`;
  };
        ...
        <table className="min-w-full table-auto border-collapse">
          ...
          <tbody>
            {users.map((u: any) => (
              <tr
                key={u.email}
                className="hover:bg-gray-50/10 border h-16"
              >
                ...
                <td className="px-2 py-1">
                  <div className="flex justify-center items-center">
                    <span
                      className={`min-w-16 px-3 py-1 text-xs font-bold rounded-full text-center ${
                        u.role === "admin"
